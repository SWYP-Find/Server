...
--
-- CHECK 제약은 엔티티의 @Check 로도 선언되어 있어 테이블 최초 생성 시 함께 만들어진다.
-- 다만 ddl-auto=update 는 이미 있는 제약을 갱신하지 않는다.
-- enum 에 값을 추가하면 아래 ALTER 를 별도로 실행해야 한다.
--   ALTER TABLE ad_creatives DROP CONSTRAINT ck_ad_creatives_slot;
--   ALTER TABLE ad_creatives ADD CONSTRAINT ck_ad_creatives_slot CHECK (slot IN (...));
-- 빠뜨리면 새 지면의 INSERT 가 조용히 막힌다. AdCheckConstraintTest 가 제약과 enum 의 불일치를 먼저 잡는다.
...
CREATE TABLE IF NOT EXISTS ad_click_logs (
    ...
    created_at  TIMESTAMP,
    updated_at  TIMESTAMP,
    CONSTRAINT ck_ad_click_logs_slot CHECK (slot IN ('HOME_FEED', 'BATTLE_RESULT_BOTTOM', 'CHAT_ROOM_INLINE', 'ATTENDANCE_COMPLETE', 'PROFILE_BOTTOM'))
);
...
CREATE TABLE IF NOT EXISTS ad_impression_daily (
    ...
    CONSTRAINT ck_ad_impression_daily_slot CHECK (slot IN ('HOME_FEED', 'BATTLE_RESULT_BOTTOM', 'CHAT_ROOM_INLINE', 'ATTENDANCE_COMPLETE', 'PROFILE_BOTTOM'))
);
