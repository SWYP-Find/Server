...
초기 지면 목록은 앱팀과 확정 후 채운다. 확정 전까지는 `HOME_FEED`, `BATTLE_RESULT_BOTTOM` 두 개로 시작한다.
...
- `AdpickLinkBuilder` — **pass-through**. 서브아이디 파라미터 규격을 아직 확인하지 못했다.
  파트너센터 링크생성 화면에서 확인 후 파라미터명만 채운다.
...
| 애드픽 파트너 가입 | 미착수 |
| 애드픽 서브아이디 파라미터 규격 확인 | 미확인. 파트너센터 링크생성 화면 |
