...
/**
 * 광고 노출 지면.
 * 테이블이 아니라 enum인 이유는, 앱이 그릴 줄 모르는 지면을 어드민에서 만들어봐야
 * 아무 일도 일어나지 않기 때문이다. 지면 추가는 어차피 앱 배포와 묶인다.
 * 목록은 iOS Presentation 모듈(Home/Battle/Chat/Profile)의 실제 화면을 기준으로 잡았다.
 * 앱팀 확정 전이므로 실제로 붙이는 지면만 소재를 등록하면 된다.
 * 소재가 없는 지면은 빈 배열을 반환하고 앱은 지면 자체를 숨긴다.
 * {@code cpiFriendly}는 앱 설치형(CPI) 광고를 놓아도 되는 지면인지를 뜻한다.
